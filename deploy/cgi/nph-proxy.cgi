...
# edit by tdm
if ( $encoded_URL eq '' ) {
	($encoded_URL) = $ENV{'QUERY_STRING'} =~ /url=(.*)/;
	$encoded_URL =~ s/\%([A-Fa-f0-9]{2})/pack('C', hex($1))/seg if $encoded_URL =~ /%3A%2F/;
	$encoded_URL =~ s#\/\/#\/#;
}
#system "echo '1 - encoded_URL - $encoded_URL' >> /tmp/proxy.log";
...
#system "echo '2 - URL - $URL' >> /tmp/proxy.log";
...
($scheme, $authority, $path)= ($URL=~ m#^([\w+.-]+)://?([^/?]*)(.*)$#i) ;
#system "echo '3 - scheme, authority, path - ($scheme, $authority, $path)' >> /tmp/proxy.log";
$scheme= lc($scheme) ;
...
#system "echo '4 - base_url - $base_url' >> /tmp/proxy.log";
...
#system "echo '5 - status, headers, body - ($status, $headers, $body)\n' >> /tmp/proxy.log";
