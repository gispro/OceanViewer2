	...
	buttons: [
		{
			...
			handler  : function(){  
				if (servicesSetting.tabs.activeTab.id === 'wmsPanel') 
					...
					servicesSetting.arcgisPanel.addRecord2Map();
				else if (servicesSetting.tabs.activeTab.id === 'animationPanel')
					Ext.getCmp('animationPanel').addRecord2Map();
				}
...
//					console.log ('Save WMS object : newObject = ' + servicesSetting.newObject);
					...
					arcgisStore.reload();  Ext.getCmp('arcgisGrid').getView().refresh();
				} else if (servicesSetting.tabs.activeTab.id === 'rssPanel'){
					if (servicesSetting.newObject === false)
			...
			getLayerSources : function ()
			{
				...
				for (var rec in app.layerSources)
				{
					if (app.layerSources[rec].baseParams && (app.layerSources[rec].baseParams.SERVICE === 'WMS') &&
					   ...
					   (app.layerSources[rec].rest_url   === Ext.getCmp("serverRestURLField").getValue()))
					{
...
//									                                         ', ptype = ' + app.layerSources[rec].ptype);
			...
			{
				Ext.getCmp("serverNameField").setValue(record.data.server_name);
				Ext.getCmp("serverURLField").setValue(record.data.url);
				Ext.getCmp("serverRestURLField").setValue(record.data.rest_url);
		...
		this.arcgisPanel = new Ext.Panel({
			...
			addRecord2Map : function()
			{
				...
				var r = Ext.getCmp("arcgisGrid").getSelectionModel().getSelected();
				...
				this.rssPanel,
				{
					...
					items	   : [ {
						...
						colModel: new Ext.grid.ColumnModel([
							//expander,
							{header: "Id", dataIndex: "anim_id", sortable: true},
							{id: "title", flex: 1, header: "Наименование", dataIndex: "title", sortable: true}							
						])						
					}],
					addRecord2Map : function() {
						...
						var layerStore = app.mapPanel.layers;
						var records = Ext.getCmp('animationGrid').getSelectionModel().getSelected();
